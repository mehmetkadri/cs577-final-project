...
def process(file_name, src_constraint, target_constraint):
    ...
    constraints = src_constraint is not None or target_constraint is not None
...
def plot(file_name, desired_src, desired_target, desired_amp, desired_margin):
    ...
    for line in lines:
        ...
        src_img = elements[8]
        margin = float(elements[9])
        amplification = float(elements[10])
        t = []
        d = []
        for i in range(0, 5):
            index = 11 + i
            t.append(elements[index])
            d.append(float(elements[index]))
...
if __name__ == "__main__":
    ...
    for fil in files:
        ...
        plot(fil_path, desired_src, desired_target, desired_amp, desired_margin)
